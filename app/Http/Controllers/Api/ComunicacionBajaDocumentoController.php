...
use App\Services\ComunicacionBajaDocumentoService;
use Illuminate\Http\Request;
use Illuminate\Validation\ValidationException;

class ComunicacionBajaDocumentoController extends Controller
{
    public function __construct(private readonly ComunicacionBajaDocumentoService $service)
    {
    }
    ...
    {
        try {
            $comunicacion = $this->service->generarPdfA4($id, $this->scope($request));
        } catch (ValidationException $e) {
            return $this->errorResponse($e, 'No se pudo generar el PDF de la comunicacion de baja.');
        }
