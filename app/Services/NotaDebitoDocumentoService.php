...
class NotaDebitoDocumentoService
{
    public function generarPdfA4(int $notaId, array $scope): NotaDebito
    {
        $nota = $this->findScoped($notaId, $scope);
        $this->validarEstadoParaDocumento($nota);
        $data = $this->obtenerDataDocumento($nota);
        $contenido = Pdf::loadView('pdf.notas-debito.nota-debito-a4', $data)->setPaper('a4')->output();

        return $this->guardarDocumento($nota, 'PDF_A4', $contenido);
    }

    public function generarTicket80(int $notaId, array $scope): NotaDebito
    {
        $nota = $this->findScoped($notaId, $scope);
        $this->validarEstadoParaDocumento($nota);
        $data = $this->obtenerDataDocumento($nota);
        $contenido = Pdf::loadView('pdf.notas-debito.nota-debito-ticket-80', $data)
    ...
    public function generarTodosFormatos(int $notaId, array $scope): NotaDebito
    {
    ...
    public function obtenerDataDocumento(NotaDebito $nota): array
    {
        $nota->loadMissing([
        ...
        return [
            ...
            'anulada' => $nota->estado === NotaDebito::ANULADA,
        ];
    }

    public function guardarDocumento(NotaDebito $nota, string $formato, string $contenido): NotaDebito
    {
        try {
    ...
    protected function validarEstadoParaDocumento(NotaDebito $nota): void
    {
        if ($nota->estado === NotaDebito::ANULADA) {
            return;
        }

        if ($nota->estado !== NotaDebito::REGISTRADA && $nota->estado_sunat !== NotaDebito::SUNAT_ACEPTADO) {
    ...
    protected function findScoped(int $notaId, array $scope): NotaDebito
    {
    ...
    protected function path(NotaDebito $nota, string $formato): string
    {
        ...
        return 'private/sunat/notas-debito/'.$nota->empresa_id.'/'.$nota->created_at->format('Y-m-d').'/'.$folder.'/'.$nota->numero_completo.'.pdf';
    }
