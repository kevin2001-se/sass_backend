...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('notas_debito', function (Blueprint $table) {
            if (! Schema::hasColumn('notas_debito', 'pdf_a4_path')) {
                $table->string('pdf_a4_path')->nullable()->after('cdr_path');
            }

            if (! Schema::hasColumn('notas_debito', 'ticket_80_path')) {
                $table->string('ticket_80_path')->nullable()->after('pdf_a4_path');
            }

            if (! Schema::hasColumn('notas_debito', 'pdf_generado_at')) {
                $table->timestamp('pdf_generado_at')->nullable()->after('rechazado_at');
            }

            if (! Schema::hasColumn('notas_debito', 'ticket_generado_at')) {
